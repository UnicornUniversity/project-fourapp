...
function OrderProvider({ children }) {
  ...
  const getAllOrders = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${env.REACT_APP_API_URL}/api/orders`, {
        method: "GET",
        credentials: "include",
        headers: {
      ...
      const userOrders = Array.isArray(data)
        ? data.filter((order) => order.customer_id === user.id)
        : [];
  ...
  const completeOrder = async (orderId, shippingDetails) => {
    ...
    try {
      const response = await fetch(
        `${env.REACT_APP_API_URL}/api/orders/${orderId}/complete`,
        {
          method: "POST",
          credentials: "include",
          headers: {
