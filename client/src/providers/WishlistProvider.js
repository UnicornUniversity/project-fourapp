...
function WishlistProvider({ children }) {
  ...
  async function handleLoad(userId) {
    try {
      const response = await fetch(
        `${env.REACT_APP_API_URL}/api/users/wishlist/${userId}`,
        {
          method: "GET",
          credentials: "include",
        }
  ...
  const addToWishlist = async (item) => {
    ...
    if (
      wishlistItems.some(
        (wishlistItem) => wishlistItem.variantId === item.variantId
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${env.REACT_APP_API_URL}/api/users/wishlist/add-item`,
        {
          method: "POST",
          headers: {
  ...
  const removeFromWishlist = async (item) => {
    try {
      const response = await fetch(
        `${env.REACT_APP_API_URL}/api/users/wishlist/remove-item`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
